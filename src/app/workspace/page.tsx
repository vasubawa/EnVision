...
const Whiteboard = dynamic(
	() =>
		import("@/components/workspace/Whiteboard").then(
			(mod) => mod.Whiteboard,
		),
	{ ssr: false },
);
	...
	return (
		<div className="w-full h-screen flex flex-col bg-transparent text-foreground overflow-hidden">
			...
			<header className="h-14 border-b border-border bg-card/60 backdrop-blur-md flex items-center justify-between px-4 shrink-0 z-50">
				...
				<div className="flex items-center gap-2">
					...
					<button
						...
						className="p-2 hover:bg-foreground/5 rounded-lg transition-colors text-foreground/60 hover:text-foreground"
						title={
							isSidebarOpen ? "Close AI Tutor" : "Open AI Tutor"
						}>
						{isSidebarOpen ? (
			...
			<main className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
				...
				<section
					className={`h-[60vh] md:h-auto ${isSidebarOpen ? "md:flex-7" : "md:flex-1"} relative border-b md:border-b-0 md:border-r border-border bg-transparent shrink-0 md:shrink transition-all duration-300`}>
					<Whiteboard />
				</section>
