...
const PROSE_WORDS = new Set([
  'to','and','or','is','are','was','were','has','have','had','do','does','did',
  'that','which','where','when','if','then','so','but','for','in','of','the',
  'a','an','on','at','by','from','with','not','can','will','would','could',
  'should','may','might','this','these','those','it','we','you','they','he','she',
  'as','such','since','because','while','although','however','therefore','thus',
  'consider','use','using','let','note','recall','find','compute','evaluate',
  'parameterize','simplify','expand','integrate','differentiate',
]);
...
function wrapBareLatex(text: string): string {
  ...
  const protected_ = text.replace(
    /\$\$[\s\S]*?\$\$|\$(?!\$)[^$\n]*?\$/g,
    (m) => { spans.push(m); return `\x00${spans.length - 1}\x00`; }
  );
  ...
  let result = out.join('');
  spans.forEach((span, i) => { result = result.replace(`\x00${i}\x00`, span); });
  return result;
}

export const MathRenderer: React.FC<MathRendererProps> = ({ content, className = '' }) => {
  // 1. Wrap bare LaTeX commands that the LLM forgot to wrap in $...$
  let preprocessed = wrapBareLatex(content);
  
  // 2. Fix the specific \int formatting edge case where the LLM might have output 
  // "\int40ydS" (missing spaces) or similar because of backslash escaping issues.
  // Add a space after \int, \iint, \iiint, \oint if it's missing.
