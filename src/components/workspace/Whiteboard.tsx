...
export function Whiteboard() {
	const { file, setGetCanvasImage, setLastCanvasUpdate } =
		useWorkspaceStore();
	...
	useEffect(() => {
		...
		if (file) {
			...
			setTimeout(() => {
				if (fabricRef.current) {
					...
					if (objs.length > 0)
						fabricRef.current.sendObjectToBack(
							objs[objs.length - 1],
						);
				}
			}, 500);
		...
		canvas.on("after:render", function () {
			if (
				document.documentElement.getAttribute("data-show-grid") !==
				"true"
			)
				return;
		...
		const getColor = () =>
			document.documentElement.getAttribute("data-draw-color") ||
			"#C05621";
		const getSize = () =>
			parseInt(
				document.documentElement.getAttribute("data-draw-size") || "4",
			);
		...
		canvas.on("mouse:down", function (opt) {
			...
			const isMultiTouch =
				evt instanceof TouchEvent && evt.touches.length > 1;
		...
		const handleMouseUp = () => {
			if (isPanning) {
				...
				canvas.defaultCursor =
					getMode() === "draw" ? "crosshair" : "default";
